...
void DDCaloDigi_BIB::processRunHeader( LCRunHeader* /*run*/) {
...
void DDCaloDigi_BIB::check( LCEvent * /*evt*/ ) { }

void DDCaloDigi_BIB::end(){
...
void DDCaloDigi_BIB::fillECALGaps( ) {
...
float DDCaloDigi_BIB::digitalHcalCalibCoeff(CHT::Layout caloLayout, float energy ) {
...
float DDCaloDigi_BIB::analogueHcalCalibCoeff(CHT::Layout caloLayout, int layer ) {
...
float DDCaloDigi_BIB::digitalEcalCalibCoeff(int layer ) {
...
float DDCaloDigi_BIB::analogueEcalCalibCoeff(int layer ) {
...
float DDCaloDigi_BIB::ecalEnergyDigi(float energy, int id0, int id1) {
...
float DDCaloDigi_BIB::ahcalEnergyDigi(float energy, int id0, int id1) {
  // some extra digi effects (daniel)
...
float DDCaloDigi_BIB::siliconDigi(float energy) {
...
float DDCaloDigi_BIB::scintillatorDigi(float energy, bool isEcal) {
...
LCCollection* DDCaloDigi_BIB::combineVirtualStripCells(LCCollection* col, bool isBarrel, int stripOrientation) {
  ...
  if ( stripOrientation==SQUARE ) {
    streamlog_out ( ERROR ) << "DDCaloDigi_BIB::combineVirtualStripCells trying to deal with silicon strips??? I do not know how to do that, refusing to do anything!" << std::endl;
    return NULL;
  }
...
int DDCaloDigi_BIB::getNumberOfVirtualCells() {
...
std::vector < std::pair <int, int> > & DDCaloDigi_BIB::getLayerConfig() {
...
void DDCaloDigi_BIB::checkConsistency(std::string colName, int layer) {
...
std::pair < int, int > DDCaloDigi_BIB::getLayerProperties( std::string colName, int layer ) {
...
int DDCaloDigi_BIB::getStripOrientationFromColName( std::string colName ) {
  int orientation(-99);
  std::string colNameLow(colName);
  std::transform(colNameLow.begin(), colNameLow.end(), colNameLow.begin(), ::tolower);
